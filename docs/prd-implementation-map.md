...
| §14.1 Model fallback chain         | `packages/ai/src/fallback.ts`, `openai.ts`, `gemini.ts` — anthropic → openai → gemini, skipped only on budget exhaustion | `packages/ai/src/fallback.test.ts` |
